...
static bool twi_master_write(uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    while (true)
    {
        ...
        if (timeout == 0 || NRF_TWI1->EVENTS_ERROR != 0)
        {
          if(timeout == 0)
          {
              PRINTS("TWI Write Timeout\r\n");
          }else{
            PRINTS("TWI Write Error: ");
            PRINT_HEX(&NRF_TWI1->EVENTS_ERROR, sizeof(NRF_TWI1->EVENTS_ERROR));
            PRINTS("\r\n");
          }
...
static bool twi_master_read(uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    while (true)
    {
        ...
        if (timeout == 0 || NRF_TWI1->EVENTS_ERROR != 0)
        {
          if(timeout == 0)
          {
              PRINTS("TWI Read Timeout\r\n");
          }else{
            PRINTS("TWI Read Error: ");
            PRINT_HEX(&NRF_TWI1->EVENTS_ERROR, sizeof(NRF_TWI1->EVENTS_ERROR));
            PRINTS("\r\n");
          }
          // Recover the peripheral as indicated by PAN 56: "TWI: TWI module lock-up." found at
